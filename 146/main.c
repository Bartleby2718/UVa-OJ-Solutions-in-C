...
void testNextSmallest(char *code, char smallest, char expected)
{
    int i = nextSmallestIndex(code, smallest);
    char actual = code[i];
    printf("Alphabetically smallest char after %c in %s: expected %c, got %c.\n", smallest, code, expected, actual);
}
...
void testGetSuccessor(char *code, char *expected)
{
    char *actual = getSuccessor(code);
    printf("After %s: expected %s, got %s.\n", code, expected, actual);
}
